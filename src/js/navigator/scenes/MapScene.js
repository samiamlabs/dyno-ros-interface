...
export default class MapScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('robot', 'assets/robot_large.png');
    this.load.image('robotMinimap', 'assets/robot_minimap_large.png');
  }
  ...
  createMap() {
    ...
    this.map.on('pointerup', pointer => {
      ...
      if (pointer.camera === this.cameras.main) {
        ...
        const distance = Math.sqrt(
          Math.pow(downX - upX, 2) + Math.pow(downY - upY, 2)
        );
    ...
    this.sys.game.canvas.addEventListener('touchmove', event => {
      ...
      if (!this.isPanning) {
        ...
        const distance = Math.sqrt(
          Math.pow(clientX - secondClientX, 2) +
            Math.pow(clientY - secondClientY, 2)
        );
        ...
        if (this.enableTouchZoom) {
          ...
          if (Math.abs(distance - this.initialTwoFingerDistance) > deadZone) {
            ...
            const zoomAdjustment =
              -scaleFactor * (this.lastTwoFingerDistance - distance) * zoom;
  ...
  createMiniMap() {
    ...
    this.minimap = this.cameras
      .add(mapPositionX, mapPositionY, this.minimapWidth, this.minimapHeight)
      .setZoom(0.4);
  ...
  sendNavigationGoal({x, y, angle = 'undefined'}) {
    ...
    const position = {
      x: mapPoint.x * resolution,
      y: -mapPoint.y * resolution,
      z: 0
    };
  ...
  updateMiniMap() {
    if (this.map.height > 0) {
      const mapZoomX =
        this.minimap.height / (this.map.height * this.map.scaleX);
      ...
      this.minimap.scrollX =
        -this.minimapWidth / 2 +
        (this.map.width / 2 - this.map._displayOriginX) * this.map.scaleX;
      this.minimap.scrollY =
        -this.minimapHeight / 2 +
        (this.map.height / 2 - this.map._displayOriginY) * this.map.scaleY;
    }
  ...
  updateCameraBorder() {
    ...
    const borderPositionX =
      camera.scrollX -
      thickness * camera.zoom +
      camera.width / 2 * (camera.zoom - 1) / camera.zoom;
    const borderPositionY =
      camera.scrollY + camera.height / 2 * (camera.zoom - 1) / camera.zoom;
  ...
  updateDatGui() {
    ...
    const point = Phaser.Math.TransformXY(
      p.x,
      p.y,
      this.map.x,
      this.map.y,
  ...
  createDatGui() {
    ...
    mini1
      .add(minimap1, 'rotation')
      .min(0)
      .step(0.01)
      .listen();
    mini1
      .add(minimap1, 'zoom', 0.1, 2)
      .step(0.1)
      .listen();
